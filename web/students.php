...
if (isAdmin()) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
        ...
        $stmt = $pdo->prepare("INSERT INTO students (full_name, birth_date, group_name, specialty, phone, email, address) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
            $_POST['full_name'], 
            $birth_date, 
    ...
    if (isset($_POST['edit_id'])) {
        $birth_date = !empty($_POST['birth_date']) ? $_POST['birth_date'] : null;
        $stmt = $pdo->prepare("UPDATE students SET full_name=?, birth_date=?, group_name=?, specialty=?, phone=?, email=?, address=? WHERE id=?");
        $stmt->execute([
            $_POST['full_name'], 
            $birth_date, 
            $_POST['group_name'] ?? null, 
...
<div class="students-page">
    <div class="page-header">
        ...
        <p class="page-description">Всего студентов: <?= count($students) ?></p>
    </div>
    
    <?php if (isset($_GET['msg'])): ?>
        <div class="alert alert-success">
            <?php if ($_GET['msg'] == 'added'): ?>
                <span class="alert-icon">✅</span> Студент успешно добавлен!
            <?php elseif ($_GET['msg'] == 'updated'): ?>
                <span class="alert-icon">✏️</span> Студент успешно обновлен!
            <?php elseif ($_GET['msg'] == 'deleted'): ?>
                <span class="alert-icon">🗑️</span> Студент успешно удален!
            <?php endif; ?>
        </div>
    <?php endif; ?>
    ...
    <?php if (isAdmin()): ?>
        <div class="admin-section">
            <button onclick="toggleForm()" class="btn btn-primary">
                <span class="btn-icon">➕</span> Добавить студента
            </button>
            
            <div id="addForm" class="add-form" style="display: none;">
                <div class="form-card">
                    <h3 class="form-title">📝 Добавление нового студента</h3>
                    <form method="POST">
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-field">
                                <label>Группа</label>
                                <input type="text" name="group_name" placeholder="Например: ТЭ-21">
                            </div>
                            <div class="form-field">
                                <label>Специальность</label>
                                <input type="text" name="specialty" placeholder="Например: Программирование">
                            </div>
                            <div class="form-field">
                                <label>Телефон</label>
                            ...
                            <div class="form-field">
                                <label>Email</label>
                                <input type="email" name="email" placeholder="student@apt.ru">
                            </div>
                            <div class="form-field full-width">
                                <label>Адрес</label>
                                <textarea name="address" rows="2" placeholder="г. Ангарск, ул. ..."></textarea>
                            </div>
                        </div>
        ...
        <table class="student-table">
            <thead>
                <tr>
                    ...
                    <th class="col-specialty">Специальность</th>
            ...
            <tbody>
                <?php if (count($students) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td class="col-birth"><?= $s['birth_date'] && $s['birth_date'] != '0000-00-00' ? date('d.m.Y', strtotime($s['birth_date'])) : '—' ?></td>
                        <td class="col-group"><?= htmlspecialchars($s['group_name'] ?? '—') ?></td>
                        <td class="col-specialty"><?= htmlspecialchars($s['specialty'] ?? '—') ?></td>
                        <td class="col-actions">
                            <!-- Кнопка просмотра для всех авторизованных -->
                            <button onclick="viewStudent(
                                '<?= addslashes($s['full_name']) ?>',
                                '<?= $s['birth_date'] ?>',
                                '<?= addslashes($s['group_name'] ?? '') ?>',
                                '<?= addslashes($s['specialty'] ?? '') ?>',
                                '<?= addslashes($s['phone'] ?? '') ?>',
                                '<?= addslashes($s['email'] ?? '') ?>',
                            ...
                            <?php if (isAdmin()): ?>
                                ...
                                <a href="?delete=<?= $s['id'] ?>" class="action-btn action-delete" onclick="return confirm('Удалить студента «<?= addslashes($s['full_name']) ?>»?')" title="Удалить">
                                    🗑️
                                </a>
                            <?php endif; ?>
                        </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="8" class="empty-state">
                            ...
                            <?php if (isAdmin()): ?>
                                <button onclick="toggleForm()" class="btn btn-primary btn-sm">➕ Добавить первого студента</button>
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endif; ?>
...
<div id="viewModal" class="modal">
    <div class="modal-content view-modal">
        <div class="modal-header">
            <h3>👁️ Просмотр информации о студенте</h3>
        ...
        <div class="modal-body">
            <div class="info-card">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value" id="view_group_name"></span>
                </div>
                <div class="info-row">
                    <span class="info-label">Специальность:</span>
                    <span class="info-value" id="view_specialty"></span>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value" id="view_address"></span>
                </div>
            </div>
        </div>
...
function viewStudent(full_name, birth_date, group_name, specialty, phone, email, address) {
    ...
    document.getElementById('view_group_name').innerText = group_name || '—';
    document.getElementById('view_specialty').innerText = specialty || '—';
...
function editStudent(id, full_name, birth_date, group_name, specialty, phone, email, address) {
